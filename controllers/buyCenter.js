...
var originPhones = process.env.filepath ? require(process.env.filepath):require("../accounts/jiangsu.json");
var fixedShopId = process.env.sid || '152520100037'; //双龙网点
...
function buyChild(number) {
  ...
  MaotaiService.getCurrentJar(tel)
      .then(j => {
        currentJar = j;
        _startTime = +new Date();
        return MaotaiService.createOrderForCenter(account, pid, quantity,300, userAgent, scopeAddress, fixedShopId, -1, currentJar)
        //return MaotaiService.createOrderByScan(tel, pid , 6, userAgent, scopeAddress, fixedShopId, currentJar);
          ...
          if(data.data !=undefined && data.data.code != undefined && data.data.code === 0){
            ...
            fs.writeFileSync(`output/${Utils.dateFormat()}_jiangsu.json`, `${tel} ${pass} ${new Date()}\n`, {flag:'a+'});
            intervalBuy();
          }
      }).catch(e => {
